...
def mha_forward_kernel(
    ...
    prefix_mask_ref: jax.Array | None,
    o_ref: Any,  # Output
    *residual_refs: Any,  # Residual outputs
    ...
    q_prefix_mask = (
        None if prefix_mask_ref is None else pl.load(prefix_mask_ref, (curr_q_slice,))
    )
    ...
    def body(start_k, carry):
        ...
        if causal or segment_ids_ref is not None:
            ...
            if causal:
                ...
                if prefix_mask_ref is not None:
                    kv_prefix_mask = pl.load(prefix_mask_ref, (curr_k_slice,))
                    causal_mask = jnp.logical_or(
                        causal_mask,
                        jnp.logical_and(
                            q_prefix_mask[:, None], kv_prefix_mask[None, :]
                        ),
                    )
    ...
    if causal and prefix_mask_ref is None:
...
def mha(
    ...
    prefix_mask: jnp.ndarray | None = None,
    sm_scale: float = 1.0,
    causal: bool = False,
    block_q: int = 128,
    block_k: int = 128,
    ...
    assert causal or prefix_mask is None, "Set causal=True when using prefix_mask"
...
def _mha_forward(
    ...
    return out, (q, k, v, segment_ids, prefix_mask, out, lse)
...
def mha_backward_kernel(
    ...
    *,
    ...
    kv_prefix_mask = (
        None if prefix_mask_ref is None else pl.load(prefix_mask_ref, (curr_k_slice,))
    )
    ...
    def inner_loop_dkdv(start_q, carry):
        ...
        if causal or segment_ids_ref is not None:
            ...
            if causal:
                ...
                if prefix_mask_ref is not None:
                    q_prefix_mask = pl.load(prefix_mask_ref, (curr_q_slice,))
                    causal_mask = jnp.logical_or(
                        causal_mask,
                        jnp.logical_and(
                            q_prefix_mask[:, None], kv_prefix_mask[None, :]
...
def _mha_backward(
    ...
    q, k, v, segment_ids, prefix_mask, out, lse = res
    ...
    if backward_pass_impl == "xla":
        ...
        in_specs = [
            ...
            (
                None
                if segment_ids is None
                else pl.BlockSpec((None, seq_len), lambda i, j, _: (i, 0))
            ),
            (
                None
                if prefix_mask is None
        ...
        )(q, k, v, segment_ids, prefix_mask, out, do, lse, delta)
    else:
